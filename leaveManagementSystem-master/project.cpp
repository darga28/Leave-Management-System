...
void displayUsers()
{
	...
	cout<<nl<<s4<<"Emp ID"<<s2<<"Name"<<s2<<"Age"<<s2<<"Salary"<<s1<<sp2<<"Experience"<<nl<<nl;
	while(file){
		getline(file,str,'|');
		cout<<str<<s1;
	}
...
void dispLeave()
{
	...
	cout<<nl<<"Emp ID\t"<<"Name\t"<<"CL(20)\t"<<"SL(15)\t"<<"STL(10)\t"<<"PL(40)\t"<<"Salary\t"<<"Start Date\t"<<sp2<<sp2<<"End Date\t"<<sp2<<sp2<<"NoofLeaves\t"<<nl<<nl;
	while(file){
		getline(file,str,'|');
		cout<<str<<"\t";
	}
...
int main()
{
	...
	if(strcmp(name,"admin")==0){
		if(pass==1234){
		flag=1;
		cout<<"login sucessfull as a ADMIN\n";
	}
	}
	if(strcmp(name,"user")==0){
		if(pass==5678){
		flag=2;
		cout<<"login sucessfull as a USER\n";
...
if(flag==1)
{
	cout<<s3<<"----- E M P L O Y E E   L E A V E   M A N A G E M E N T  S Y S T E M -----"<<nl<<nl;
	cout<<s6<<"--- M A I N  M E N U ---"<<nl<<nl;
	cout<<s4<<"1. DELETE LEAVE"<<nl;
	cout<<s4<<"2. SEARCH LEAVE"<<nl;
	cout<<s4<<"3. MODIFY LEAVE"<<nl;
	cout<<s4<<"4. DISPLAY ALL LEAVES"<<nl;
	cout<<s4<<"5. USER MANAGEMENT"<<nl;
	cout<<s4<<"6. EXIT"<<nl;
	cout<<nl<<s4<<cyp;
	cin>>ch;
	cout<<nl;
